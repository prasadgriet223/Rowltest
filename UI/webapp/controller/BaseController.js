	...
	return Controller.extend("scp.com.saparate.controller.BaseController", {
		...
		loadDatatoViewwithKey_GET: function (sProperty, sView, sKey) {
			var sHeaders = {
				...
				"xu": sap.ui.getCore().getModel('oKeyModel').getProperty("/saparate/email")
			};
		...
		breadCrumbNavigate: function (obreadCrumb,from, key) {
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			if (key === "1") {
				oRouter.navTo("Dashboard");
			} else if (key === "2" && from === "fromBuildStages") {
				oRouter.navTo("jobs");
			} else if (key === "3" && from === "fromBuildStages") {
				oRouter.navTo("Builds", {
					jobId: obreadCrumb.getLinks()[3].getText()
				});
			} else if (key === "4" && from === "fromBuildStages") {
				oRouter.navTo("buildStages", {
					jobId: obreadCrumb.getLinks()[3].getText(),
					buildid:obreadCrumb.getLinks()[4].getText()
				});
			}
		}
	});
